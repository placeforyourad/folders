...
Результат — **массив путей** от `root` до каждого найденного элемента, без соседних веток. Если один найденный элемент является предком другого найденного элемента, в результат попадает только путь до более глубокого совпадения — путь до предка-совпадения в него уже включён.
